    ...
    <main className="mx-auto min-h-screen max-w-6xl p-4">
      ...
      <section className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4 text-sm">
        <ul className="space-y-2">
          {rows.map((row) => (
            <li key={row.name} className="flex items-center justify-between rounded border border-white/10 bg-white/5 p-2">
              <span className="text-[var(--text)]">{row.name}</span>
              <span className="text-[var(--accent)]">${row.price}</span>
              <button className="rounded border border-white/10 px-2 py-1 text-[var(--subtext)]">{row.active ? 'Activo' : 'Inactivo'}</button>
            </li>
          ))}
        </ul>
      </section>
